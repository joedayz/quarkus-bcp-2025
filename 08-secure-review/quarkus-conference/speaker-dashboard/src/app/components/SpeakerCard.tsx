...
const icons = [<SpeakIcon key="speaker-icon-1" size="sm" color="#aaaa22" />];
...
export function SpeakerCard({ speaker }: SpeakerCardProps): JSX.Element {
  return (
    <Card isFlat>
      <CardTitle>
        {getSpeakerIcon(speaker)} {speaker.nameFirst}{" "}
      </CardTitle>
      <CardBody>
        <DescriptionList>
          <DescriptionListGroup>
...
function getSpeakerIcon(speaker: Speaker) {
  return (
    icons[speaker.id - 1] || (
      <SpeakIcon key="speaker-icon-1" size="md" color="#ffaa22" />
    )
  );
}
